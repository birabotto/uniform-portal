...
function buildShell({ title, eyebrow, bodyHtml, footerNote }) {
  ...
  <div style="margin:0;padding:24px;background:#f3f6fb;font-family:Arial,Helvetica,sans-serif;color:#0f172a;">
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid #dbe4f0;box-shadow:0 14px 34px rgba(15,23,42,0.08);">
      ...
      <div style="padding:18px 32px;background:#f8fafc;border-top:1px solid #e2e8f0;color:#64748b;font-size:12px;line-height:1.6;">${footerNote || 'This email was sent automatically by the IKEA Uniform Portal.'}</div>
    </div>
  </div>`
}
...
function buildNewRequestEmail(payload) {
  ...
  return buildShell({
    ...
    eyebrow: 'New request',
    bodyHtml: `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.7;color:#334155;">A new employee request has been submitted and is ready for admin review.</p>
      <div style="display:inline-block;background:${statusTone};color:${textTone};padding:10px 14px;border-radius:999px;font-size:13px;font-weight:700;text-transform:capitalize;">Status: ${escapeHtml(payload.status || 'pending')}</div>
      ${buildSummaryGrid([
...
function buildStatusChangeEmail(payload) {
  ...
  return buildShell({
    ...
    bodyHtml: `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.7;color:#334155;">Hello <strong>${escapeHtml(payload.employee_name)}</strong>, your uniform request has been updated.</p>
      <div style="display:inline-block;background:${badgeBackground};color:${badgeColor};padding:10px 14px;border-radius:999px;font-size:13px;font-weight:700;text-transform:capitalize;">New status: ${escapeHtml(payload.status)}</div>
      ${buildSummaryGrid([
        { label: 'Product', value: payload.product_type },
        { label: 'Style', value: payload.style },
        { label: 'Colour', value: payload.colour },
