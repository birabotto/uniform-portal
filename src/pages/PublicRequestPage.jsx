...
export default function PublicRequestPage() {
    ...
    async function loadInventory() {
      ...
      const [{ data: inventoryData, error: loadError }, { data: sizeData }] = await Promise.all([
        supabase
          ...
          .select('id, product_type, colour, style, size, stock_quantity, reserved_quantity, is_active')
          .eq('is_active', true)
          .order('product_type')
          .order('style')
          .order('colour'),
  ...
  const productTypes = useMemo(() => [...new Set(inventory.map((item) => item.product_type))], [inventory])
  const styles = useMemo(
    () => [...new Set(inventory.filter((item) => !form.product_type || item.product_type === form.product_type).map((item) => item.style))],
    [inventory, form.product_type]
  )
  const colours = useMemo(
    () => [...new Set(inventory.filter((item) => (!form.product_type || item.product_type === form.product_type) && (!form.style || item.style === form.style)).map((item) => item.colour))],
  ...
  const sizeRows = useMemo(() => {
    return inventory
      .filter((item) =>
        ...
        (!form.colour || item.colour === form.colour)
      )
      .sort((a, b) => sizeOrder.indexOf(a.size) - sizeOrder.indexOf(b.size))
  }, [inventory, form.product_type, form.style, form.colour])

  const exactAvailability = inventory.find(
    (item) =>
  ...
  function updateField(event) {
    ...
    setForm((current) => ({
      ...current,
      [name]: value,
      ...(name === 'product_type' ? { style: '', colour: '', requested_size: '' } : {}),
      ...(name === 'style' ? { colour: '', requested_size: '' } : {}),
    ...
    if (name === 'employee_id') {
      ...
      setEmployee(null)
      setEmployeeLookupState(value.trim() ? 'idle' : 'idle')
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    const { data, error: rpcError } = await supabase.rpc('submit_uniform_request', {
      ...
      p_style: form.style,
      p_colour: form.colour,
      p_requested_size: form.requested_size,
    ...
    try {
      await sendNewRequestEmail({
        ...
        style: form.style,
        colour: form.colour,
        requested_size: form.requested_size,
  ...
  const freeUnits = exactAvailability ? Math.max(exactAvailability.stock_quantity - exactAvailability.reserved_quantity, 0) : 0
          ...
          <div className="grid gap-6 px-5 py-8 sm:px-8 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm text-blue-100 sm:text-base">
                Enter your employee ID to auto-fill your IKEA profile, then choose the item, style, colour, and size you need. Stock is reserved while the request stays pending.
              </p>
            </div>
            <div className="grid gap-3 rounded-[28px] bg-white/10 p-4 backdrop-blur sm:grid-cols-3 lg:grid-cols-1">
              ...
              <div>
                ...
                <div className="mt-2 font-semibold">Review stock and size chart</div>
              </div>
              <div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_360px]">
          <div className="card overflow-hidden">
            <div className="border-b border-slate-100 px-5 py-5 sm:px-8">
              ...
              <p className="mt-1 text-sm text-slate-500">The employee directory drives the auto-filled name and IKEA email fields below.</p>
            </div>
            ...
            <div className="p-5 sm:p-8">
              ...
              <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <select className="field" name="style" value={form.style} onChange={updateField} required>
                    <option value="">Select</option>
                ...
                <div>
                  ...
                  <select className="field" name="colour" value={form.colour} onChange={updateField} required>
                    <option value="">Select</option>
                ...
                <div>
                  ...
                  <select className="field" name="requested_size" value={form.requested_size} onChange={updateField} required>
                    <option value="">Select</option>
                    {sizeRows.map((item) => <option key={item.id} value={item.size}>{item.size}</option>)}
                  </select>
                </div>
                <div className="md:col-span-2">
          ...
          <div className="space-y-6">
            <div className="card p-6">
              <h2 className="text-lg font-semibold text-ink">Live availability</h2>
              <p className="mt-2 text-sm text-slate-500">Preview the exact selected combination before submitting.</p>
              <div className="mt-4 rounded-[24px] bg-slate-50 p-4 text-sm">
                {!form.requested_size ? (
                  <div className="text-slate-500">Select a product, style, colour, and size to preview inventory.</div>
                ) : exactAvailability ? (
            ...
            <div className="card p-6">
              <h2 className="text-lg font-semibold text-ink">Sizes and stock by combination</h2>
              <div className="mt-4 space-y-3 text-sm">
                {sizeRows.length ? sizeRows.map((item) => {
                  const available = Math.max((item.stock_quantity || 0) - (item.reserved_quantity || 0), 0)
                  return (
                    <label key={item.id} className={`block rounded-[24px] border p-4 transition ${form.requested_size === item.size ? 'border-ikeaBlue bg-blue-50' : 'border-slate-200 bg-white'}`}>
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <div className="font-semibold text-ink">Size {item.size}</div>
                ...
                }) : <div className="rounded-2xl bg-slate-50 p-4 text-slate-500">Select a product, style, and colour to load the available sizes.</div>}
              </div>
            </div>

            <div className="card p-6">
              <h2 className="text-lg font-semibold text-ink">Uniform size chart</h2>
              <p className="mt-2 text-sm text-slate-500">Quick reference for co-workers before they submit a request.</p>
              <div className="mt-4 space-y-3 text-sm">
                {sizeChart.map((row) => (
                  <div key={row.id} className="rounded-[24px] border border-slate-200 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-slate-500">Chest {row.chest_range || '—'} · Waist {row.waist_range || '—'}</div>
                      </div>
                      <Badge tone="healthy">{row.fit_note || 'Standard fit'}</Badge>
                    </div>
                  </div>
                ))}
              </div>
