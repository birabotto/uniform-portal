  ...
  const filteredRequests = useMemo(() => {
    return requests.filter((request) => {
      const lineText = getLines(request)
        .map((line) => `${line.product_type || ''} ${line.style || ''} ${line.colour || ''} ${line.requested_size || ''}`)
        .join(' ')
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5">
        <div className="grid gap-4 lg:grid-cols-[1fr_220px]">
          <input className="field" placeholder="Search by employee ID, employee, email, department, product, style..." value={query} onChange={(e) => setQuery(e.target.value)} />
                    ...
                    {getLines(request).map((line) => (
                      <div key={line.id} className="rounded-2xl bg-white p-4">
                        <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          <div><span className="font-semibold text-ink">Style:</span> {line.style || '—'}</div>
