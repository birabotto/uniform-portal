...
function getRequestLines(request) {
  ...
  if (request.product_type || request.fit || request.colour || request.requested_size) {
    return [
      {
...
function makeItemLabel(item) {
  return [item?.product_type, item?.fit, item?.requested_size || item?.size].filter(Boolean).join(' · ') || 'Item details unavailable'
}
...
export default function AdminDashboardPage() {
  ...
  async function load() {
    ...
    const [
      ...
      supabase.from('inventory_items').select('*').order('product_type').order('fit'),
  ...
  const derivedAnalytics = useMemo(() => {
    ...
    requests.forEach((request) => {
      ...
      getRequestLines(request).forEach((line) => {
        const key = [line.product_type, line.fit, line.requested_size].join('||')
  ...
  const topRequestedSizes = useMemo(() => {
    const counts = analytics.reduce((acc, row) => {
      const size = row.size ?? row.requested_size ?? row.size_label ?? 'Unknown'
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Track request demand, IKEA size codes, most ordered products, employee activity, and low-stock risks in one dashboard."
      />
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <div className="card p-6">
          ...
          <p className="mt-1 text-sm text-slate-500">Use this to decide which IKEA size codes should be replenished first.</p>
          <div className="mt-5">
            <SimpleBarChart items={topRequestedSizes} emptyLabel="IKEA size codes will show up after employees submit requests." />
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <div className="card p-6">
          <h2 className="text-xl font-semibold text-ink">Most requested products</h2>
          <p className="mt-1 text-sm text-slate-500">Top products and fit combinations requested by co-workers.</p>
          <div className="mt-5">
                ...
                lowStockItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="rounded-[24px] border border-slate-200 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <div className="font-semibold text-ink">{item.product_type} · {item.fit || '—'}</div>
                          <div className="text-sm text-slate-500">{item.size}{item.location ? ` · ${item.location}` : ''}</div>
                        </div>
